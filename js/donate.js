...
(function(){
  var donateCtrl, app;
  donateCtrl = function($scope, $route){
    var objs;
    objs = [
      {
        t: '自學法制化',
    ...
    angular.extend($scope, {
      cumuRoute: function(n){
        var rs, obj, cu, act, comp;
        rs = ['\\', 'gift', 'notify', 'thanks'];
        obj = {};
        cu = $route && $route.current && $route.current.templateUrl;
        cu = cu && cu.replace(/^.*\//, '').replace(/\.html$/, '');
        act = cu && cu.indexOf(n) > -1;
        obj.active = act;
  ...
  app = angular.module('donateApp', ['ngRoute']).controller('donateCtrl', ['$scope', '$route', donateCtrl]);
